...
def custom_grid_sample(tensor: torch.Tensor, res_grid: torch.Tensor) -> torch.Tensor:
    ...
    xs = torch.linspace(0.5, tensor.shape[-1] - 0.5, steps=tensor.shape[-1])
    xs = xs / tensor.shape[-1] * 2 - 1
    ys = torch.linspace(0.5, tensor.shape[-2] - 0.5, steps=tensor.shape[-2])
    ys = ys / tensor.shape[-2] * 2 - 1
...
class DecoderBlock(nn.Module):
    def __init__(self, dim: int, coarse_feat_dim: bool, search_range: int = 2) -> None:
        ...
        concat_dim = (
            dim * (search_range * search_range * 4 + 1)
            + coarse_feat_dim
            + (2 if coarse_feat_dim > 0 else 0)
        )
    ...
    def forward(
        ...
        c1: torch.Tensor,
        c2: torch.Tensor,
        coarse_flow: Optional[torch.Tensor],
