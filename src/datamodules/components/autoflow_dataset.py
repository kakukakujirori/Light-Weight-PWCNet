...
from src.datamodules.components.augmentations import (
...
def warp(
    ...
    if fg_mask is None:
        warped_img, flow = randomPerspective(img, scale=(0.01, 0.025))
        return warped_img, None, flow
    else:
        fg_mask = fg_mask.astype(np.uint8)
        # TODO: Try other geometric transformations, should we add an additional Euclid transform?
        [warped_img, warped_mask], flow = randomGridDistortion(
            [img, fg_mask], num_steps=(2, 5), distort_limit=0.05
        )
        return warped_img, warped_mask.astype(bool), flow
